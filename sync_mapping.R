...
# get the input data
infile <- "Exempt_Organizations_List"
data <- read.csv(paste0('./', infile, '.csv'))

# combine street with city & state for best matching ability 
data <- data %>% 
  mutate(address = paste(STREET, CITY, STATE, sep = ', '))

# get the address list, and append US to the end to increase accuracy 
...
getGeoDetails <- function(address){   
  ...
  # return NA if we didn't get a match:
